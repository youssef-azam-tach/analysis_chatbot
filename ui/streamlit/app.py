...
if page == "🏠 Home":
    ...
    if not st.session_state.multi_file_loader or len(st.session_state.multi_file_loader.get_loaded_files()) == 0:
        ...
        with main_tabs[3]:
            ...
            st.info("💡 Add calculated columns to your latest dataset. These operations work on the active dataset state.")
            
            # Get available datasets
            all_datasets = get_available_datasets_for_stage('custom_columns')
            
            if not all_datasets:
                st.warning("⚠️ No datasets available. Please clean, merge, or append data first.")
            else:
                st.markdown("**📋 Column Comparison:**")
                all_cols = set()
                for table in selected_tables:
                ...
                for col in sorted(all_cols):
                    tables_with_col = [t for t in selected_tables if col in all_datasets[t].columns]
                    status = "✅ All tables" if len(tables_with_col) == len(selected_tables) else f"⚠️ {len(tables_with_col)}/{len(selected_tables)} tables"
                    col_status.append({"Column": col, "Status": status})
                ...
                if st.button("📊 Append Tables", key="execute_append"):
                    try:
                        ...
                        st.markdown("---")
                        st.markdown("### 📊 Power Query Editor - Choose Data to Load")
                        st.info("💡 Similar to Power Query in Excel - choose which data you want to work with next")
                        
                        col1, col2, col3 = st.columns(3)
                        
                        with col1:
                            if st.button("📄 Load Original Tables", use_container_width=True, key="load_original_append"):
                                for table in selected_tables:
                                    st.session_state.cleaned_datasets[f"{table}_original"] = all_datasets[table].copy()
                                st.success(f"✅ Loaded all {len(selected_tables)} original tables")
                                st.rerun()
                        
                        with col2:
                            if st.button("📋 Load Appended Only", use_container_width=True, key="load_appended_only"):
                                st.session_state.cleaned_datasets[appended_name] = appended_df.copy()
                                st.success(f"✅ Saved '{appended_name}' to cleaned datasets!")
                                st.info(f"💡 This contains all {len(selected_tables)} tables stacked together")
                                st.rerun()
                        
                        with col3:
                            if st.button("📊 Load Both", use_container_width=True, key="load_both_append"):
                                for table in selected_tables:
                                    st.session_state.cleaned_datasets[f"{table}_original"] = all_datasets[table].copy()
                                st.session_state.cleaned_datasets[appended_name] = appended_df.copy()
                                st.success(f"✅ Loaded all original tables and appended result")
                                st.rerun()
                    except Exception as e:
                        st.error(f"❌ Append failed: {str(e)}")
