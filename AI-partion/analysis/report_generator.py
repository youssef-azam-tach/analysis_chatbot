...
def generate_strategic_pdf(report_data, output_path):
    ...
    # Charts
    if 'chart_images' in report_data and report_data['chart_images']:
        pdf.add_page()
        pdf.add_section_header('RECOMMENDED CHARTS')
        for chart in report_data['chart_images']:
            caption = chart.get('caption', 'Chart')
            image_path = chart.get('path', '')
            insight_text = chart.get('insight', '')
            if image_path and os.path.exists(image_path):
                pdf.set_font('helvetica', 'B', 11)
                pdf.multi_cell(0, 7, caption, 0, 'L')
                y_start = pdf.get_y()
                if y_start > 220:
                    pdf.add_page()
                    y_start = pdf.get_y()
                pdf.image(image_path, x=15, y=y_start + 2, w=180)
                pdf.ln(82)
                if insight_text:
                    pdf.set_font('helvetica', 'I', 10)
                    cleaned = str(insight_text).replace('Business Insight:', '').strip()
                    pdf.multi_cell(0, 6, f"Business Insight: {cleaned}", 0, 'L')
                    pdf.ln(2)
