...
def _st_msg(level: str, msg: str):
    ...
    if st:
        ...
        getattr(_logger, "warning" if level == "warning" else "error" if level == "error" else "info")(msg)
...
class MultiFileLoader:
    ...
    def add_file(self, file_path: str, file_name: str = None) -> bool:
        ...
        try:
            ...
            if not Path(file_path).exists():
                _st_msg("error", f"❌ File not found: {file_path}")
                return False
            
            # Validate file size
            file_size = Path(file_path).stat().st_size / (1024 * 1024)  # MB
            if file_size > 1024:  # 1GB
                _st_msg("error", f"❌ File too large: {file_size:.1f}MB (max 1GB)")
                return False
            ...
            try:
                ...
                _st_msg("error", f"❌ Cannot open Excel file {file_name}: {str(load_err)}")
                _st_msg("info", "Possible issue: File may be corrupted or not a valid Excel format")
                return False
            ...
            try:
                ...
                _st_msg("error", f"❌ Cannot read sheets from {file_name}: {str(sheet_err)}")
                return False
            
            if not sheets:
                _st_msg("error", f"❌ No sheets found in {file_name}")
                return False
            ...
            for sheet in sheets:
                try:
                    ...
                    _st_msg("warning", f"⚠️ Cannot load sheet '{sheet}' from {file_name}: {str(sheet_load_err)}")
                    continue
            
            if loaded_count == 0:
                _st_msg("error", f"❌ No valid sheets could be loaded from {file_name}")
                return False
            ...
            _st_msg("error", f"❌ Unexpected error loading file {file_name}: {str(e)}")
            import traceback
            _st_msg("info", f"Technical details: {traceback.format_exc()}")
            return False
    ...
    def load_file(self, uploaded_file) -> bool:
        ...
        try:
            ...
            # Sanitize filename - keep only alphanumeric, spaces, dots, underscores, hyphens
            safe_filename = re.sub(r'[^\w\s\.\-]', '_', uploaded_file.name)
            
            # Save file to disk with sanitized name
            file_path = os.path.join(data_dir, safe_filename)
            with open(file_path, "wb") as f:
                f.write(uploaded_file.getbuffer())
            
            # Use original name for display, sanitized path for storage
            return self.add_file(file_path, uploaded_file.name)
        except Exception as e:
            _st_msg("error", f"❌ Error saving uploaded file: {str(e)}")
            import traceback
            _st_msg("info", f"Technical: {traceback.format_exc()}")
            return False
    ...
    def combine_sheets(self, file_sheet_pairs: List[Tuple[str, str]], 
        ...
        try:
            ...
            if not dfs:
                _st_msg("error", "No valid sheets selected")
                return None
            ...
            if how == 'concat':
                ...
                if on is None:
                    _st_msg("error", "Please specify 'on' column for merge operation")
                    return None
                combined = dfs[0]
                for df in dfs[1:]:
                    combined = combined.merge(df, on=on, how='outer')
            else:
                _st_msg("error", f"Unknown combine method: {how}")
                return None
            ...
            _st_msg("error", f"Error combining sheets: {str(e)}")
            return None
    ...
    def combine_all_files(self, how: str = 'concat', on: str = None) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if not file_sheet_pairs:
                _st_msg("error", "No files to combine")
                return None
            
            return self.combine_sheets(file_sheet_pairs, how=how, on=on)
        
        except Exception as e:
            _st_msg("error", f"Error combining all files: {str(e)}")
            return None
    ...
    def combine_by_sheet_name(self, sheet_name: str, how: str = 'concat', 
        ...
        try:
            ...
            if not file_sheet_pairs:
                _st_msg("error", f"Sheet '{sheet_name}' not found in any file")
                return None
    ...
    def export_combined(self, output_path: str, format: str = 'excel') -> bool:
        ...
        try:
            if self.combined_df is None:
                _st_msg("error", "No combined data to export")
                return False
            ...
            if format == 'excel':
                ...
                _st_msg("error", f"Unknown format: {format}")
                return False
            
            _st_msg("success", f"✅ Exported to {output_path}")
            return True
        
        except Exception as e:
            _st_msg("error", f"Error exporting: {str(e)}")
            return False
