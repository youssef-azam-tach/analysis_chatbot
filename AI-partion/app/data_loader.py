...
class ExcelLoader:
    ...
    def get_sheets(self) -> List[str]:
        ...
        try:
            ...
            _st_msg("error", f"Error reading Excel file ({self.file_path.suffix}): {str(e)}")
            if "xlrd" in str(e).lower():
                _st_msg("info", "💡 Tip: Try installing xlrd: `pip install xlrd`")
            return []
    ...
    def load_sheet(self, sheet_name: str) -> pd.DataFrame:
        ...
        try:
            ...
            _st_msg("error", f"Error loading sheet '{sheet_name}': {str(e)}")
            return None
...
def load_excel(file_path: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        _st_msg("error", f"Error loading Excel file: {str(e)}")
        return None
